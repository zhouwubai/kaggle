...
from mask_rcnn.keras.config import Config
from mask_rcnn.keras import model, data

# Path to trained weights file
COCO_MODEL_PATH = os.path.join(ROOT_DIR, "data/mask_rcnn/mask_rcnn_coco.h5")

# Directory to save logs and model checkpoints, if not provided
# through the command line argument --logs
DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR, "data/mask_rcnn/logs")
...
class CocoDataset(data.Dataset):
    ...
    def auto_download(self, dataDir, dataType, dataYear):
        ...
        dataYear: What dataset year to load (2014, 2017) as a string,
            not an integer
        Note:
        ...
        if dataType == "minival" or dataType == "valminusminival":
            ...
            imgURL = "http://images.cocodataset.org/zips/{}{}.zip".format(
                dataType, dataYear)
        # print("Image paths:"); print(imgDir);
        # print(imgZipFile); print(imgURL)
        ...
        if dataType == "minival":
            ...
            annURL = ("https://dl.dropboxusercontent.com"
                      "/s/o43o90bna78omob/instances_minival2014.json.zip?dl=0")
            unZipDir = annDir
        elif dataType == "valminusminival":
            annZipFile =\
                "{}/instances_valminusminival2014.json.zip".format(dataDir)
            ...
            unZipDir = dataDir
        # print("Annotations paths:"); print(annDir);
        # print(annFile); print(annZipFile); print(annURL)
        ...
        if not os.path.exists(annFile):
            if not os.path.exists(annZipFile):
                print("Downloading zipped annotations to " +
                      annZipFile + " ...")
                with urllib.request.urlopen(annURL) as resp:
...
def evaluate_coco(model, dataset, coco,
                  eval_type="bbox", limit=0, image_ids=None):
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        # Validation dataset
        dataset_val = CocoDataset()
        coco = dataset_val.load_coco(args.dataset, "minival",
                                     year=args.year, return_coco=True,
