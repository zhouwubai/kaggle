...
import re
from relevance.utils import ngram
import cPickle
import numpy as np
from relevance.utils.nlp_utils import stopwords, english_stemmer, stem_tokens
from relevance.utils.feat_utils import try_divide, dump_feat_name
from relevance.config import config
from relevance.constants import token_pattern
...
def extract_feat(df):
    ...
    for feat_name in feat_names:
        for gram in grams:
            ...
            df["count_of_%s_%s" % (feat_name, gram)] =\
                list(df.apply(lambda x: len(x[feat_name + "_" + gram]),
                              axis=1))
            df["count_of_unique_%s_%s" % (feat_name, gram)] =\
                list(df.apply(lambda x: len(set(x[feat_name + "_" + gram])),
                              axis=1))
        ...
        # digit count
        df["count_of_digit_in_%s" % feat_name] =\
            list(df.apply(lambda x: count_digit(x[feat_name + "_unigram"]), axis=1))
        df["ratio_of_digit_in_%s" % feat_name] =\
            list(map(try_divide,
                     df["count_of_digit_in_%s" % feat_name],
        ...
        for obs_name in feat_names:
            for target_name in feat_names:
                if target_name != obs_name:
                    # query
                    df["count_of_%s_%s_in_%s" % (obs_name, gram, target_name)] =\
                        list(df.apply(lambda x: sum([1. for w in x[obs_name + "_" + gram]
                                                    if w in set(x[target_name + "_" + gram])]), axis=1))
                    df["ratio_of_%s_%s_in_%s" % (obs_name, gram, target_name)] =\
                        list(map(try_divide,
                                 df["count_of_%s_%s_in_%s" % (obs_name, gram, target_name)],
                                 df["count_of_%s_%s" % (obs_name, gram)]))

        # some other feat
        df["title_%s_in_query_div_query_%s" % (gram, gram)] =\
            list(map(try_divide,
                     df["count_of_title_%s_in_query" % gram],
                     df["count_of_query_%s" % gram]))
        df["title_%s_in_query_div_query_%s_in_title" % (gram, gram)] =\
            list(map(try_divide,
                     df["count_of_title_%s_in_query" % gram],
                     df["count_of_query_%s_in_title" % gram]))
        df["description_%s_in_query_div_query_%s" % (gram, gram)] =\
            list(map(try_divide,
                     df["count_of_description_%s_in_query" % gram],
                     df["count_of_query_%s" % gram]))
        df["description_%s_in_query_div_query_%s_in_description" % (gram, gram)] =\
            list(map(try_divide,
                     df["count_of_description_%s_in_query" % gram],
    ...
    for gram in grams:
        for target_name in feat_names:
            for obs_name in feat_names:
                if target_name != obs_name:
                    pos = list(df.apply(lambda x: get_position_list(x[target_name + "_" + gram],
...
if __name__ == "__main__":
    ...
    feat_names = [
        ...
        if "count" in name or "ratio" in name or
        "div" in name or "pos_of" in name
    ]
