...
__description__
    ...
    This file generates the following features for each run and fold, and for the entire training and testing set.
        ...
        3. cosine similarity stats features for title/description
            ...
            - stats features include quantiles of cosine similarity and others defined
              in the variable "stats_func", e.g.,
                - mean value
...
from relevance.utils.nlp_utils import getTFV, getBOW
from relevance.utils.feat_utils import get_sample_indices_by_relevance, dump_feat_name
...
def generate_dist_stats_feat(metric, X_train, ids_train,
                             X_test, ids_test, indices_dict, qids_test=None):
    if metric == "cosine":
    ...
    for feat_name, column_name in zip(feat_names, column_names):
        ...
        if stats_feat_flag:
            ...
            if column_name in ["product_title", "product_description"]:
                ...
                # train
                cosine_sim_stats_feat_by_relevance_train =\
                    generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values,
                                             X_train, dfTrain["id"].values,
                                             relevance_indices_dict)
                cosine_sim_stats_feat_by_query_relevance_train =\
                    generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values,
                                             X_train, dfTrain["id"].values,
                                             query_relevance_indices_dict,
                                             dfTrain["qid"].values)
                with open("%s/train.%s_cosine_sim_stats_feat_by_relevance.feat.pkl" %
                          (path, feat_name), "wb") as f:
                    cPickle.dump(cosine_sim_stats_feat_by_relevance_train, f, -1)
                with open("%s/train.%s_cosine_sim_stats_feat_by_query_relevance.feat.pkl" %
                          (path, feat_name), "wb") as f:
                    cPickle.dump(cosine_sim_stats_feat_by_query_relevance_train, f, -1)
                # test
                cosine_sim_stats_feat_by_relevance_test =\
                    generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values,
                                             X_test, dfTest["id"].values,
                                             relevance_indices_dict)
                cosine_sim_stats_feat_by_query_relevance_test =\
                    generate_dist_stats_feat("cosine", X_train, dfTrain["id"].values,
                                             X_test, dfTest["id"].values,
                                             query_relevance_indices_dict,
                                             dfTest["qid"].values)
                ...
                with open("%s/%s.%s_cosine_sim_stats_feat_by_query_relevance.feat.pkl" %
    ...
    for i in range(len(feat_names) - 1):
        for j in range(i + 1, len(feat_names)):
            ...
            for mod in ["train", mode]:
                ...
                # dump feat
                with open("%s/%s.%s_%s_%s_cosine_sim.feat.pkl" % (path, mod, feat_names[i],
                                                                  feat_names[j], vec_type), "wb") as f:
        ...
        for feat_name, column_name in zip(feat_names, column_names):
            ...
            if stats_feat_flag:
                ...
                if column_name in ["product_title", "product_description"]:
                    ...
                    # train
                    cosine_sim_stats_feat_by_relevance_train =\
                        generate_dist_stats_feat("cosine", X_svd_train, dfTrain["id"].values,
                    ...
                    # update feat names
                    new_feat_names.append("%s_common_svd%d_cosine_sim_stats_feat_by_relevance" %
                                          (feat_name, n_components))
                    new_feat_names.append("%s_common_svd%d_cosine_sim_stats_feat_by_query_relevance" %
                                          (feat_name, n_components))
            ...
            for j in range(i + 1, len(feat_names)):
                ...
                for mod in ["train", mode]:
                    ...
                    with open("%s/%s.%s_%s_%s_common_svd%d_cosine_sim.feat.pkl" %
                        ...
                        cPickle.dump(sim, f, -1)
                # update feat names
                new_feat_names.append("%s_%s_%s_common_svd%d_cosine_sim" %
        ...
        for feat_name, column_name in zip(feat_names, column_names):
            ...
            if stats_feat_flag:
                ...
                if column_name in ["product_title", "product_description"]:
                    ...
                    # update feat names
                    new_feat_names.append("%s_individual_svd%d_cosine_sim_stats_feat_by_relevance" %
                                          (feat_name, n_components))
                    new_feat_names.append("%s_individual_svd%d_cosine_sim_stats_feat_by_query_relevance" %
                                          (feat_name, n_components))
